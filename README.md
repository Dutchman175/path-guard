...
**Towers (cheap → pricey):** Barrage (swarm shredder), Beacon (slow aura), Archer, **Barrier** (path-only full stop 3s + cooldown), Frost, **Bank** (+gold each wave clear, no shots), Cannon (splash), Sniper (map-wide), Zap (multi). Streak bounties every 5 kills.
...
**UX (build 44+):** **Barrier** path-halt tower; hide controls until map picked; FAQ; cost-sorted bar; Bank income; 10 lives; 3x speed. Build 43 UX retained.
