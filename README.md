...
**Towers (cheap → pricey):** Beacon (slow aura), Archer, **Barrier** (path-only full stop 3s + cooldown), Frost, **Bank** (+gold each wave clear, no shots), Cannon (splash), Sniper (map-wide), Zap (multi). Streak bounties every 5 kills.
...
**UX (build 48+):** Compact phone controls (scrollable tower strip + denser util/wave chrome); Choose Map scroll no longer picks a map (tap only); Barrage removed; **Barrier** path-halt; hide controls until map picked; FAQ; cost-sorted bar; Bank income; 10 lives; 3x speed; **Crucible** nightmare map.
