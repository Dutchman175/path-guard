...
Gold from kills + **Bank** income only (no interest, early-call, perfect-clear, or challenge bonuses). Waves around 5 / 10 / 15 / 20 / 25 / 30 / 35 / 40 stay denser/tougher as normal hard waves. **Double-tap** a tower to upgrade. Sell confirms. Speed cycles 1x → 2x → 3x → 5x. Start with **10 lives**.

**UX (build 65):** Inline multi-row tower-menu info (Back + selected; other slots → name/cost/dmg·rate or type-specific stats/?/×). No floating tip; no `#tower-stats` while picking from the bar. **Per-tower Aim** on Upgrade/Sell row (First/Strong/Close). Util row: Speed · Pause · Maps · FAQ (FX toggle removed — particles/shake always on). Fire = constant flame. Sprite `?v=75`. Build 75: Grunt one-shot by Archer L1; wave 1 stays small/Grunt-only. Bazooka = huge single-target rocket (gunnery).
